...
const ItemCard: React.FC<ItemCardProps> = ({ brandName, productName }) => {
  return (
    <Grid item className='flex flex-col w-[23%] h-[300px] border-solid border-2 border-gray-200 rounded-[20px] overflow-hidden cursor-pointer'>
      {/* <img className="w-full min-h-[75%] object-cover" src='assets/images/placeHolderImage.jpeg' alt='placeholder' /> */}
      <div className="w-full min-h-[75%] object-cover"/>
      {/* Item Main Desc */}
      <Box className="flex w-full p-4 justify-start items-center">
        ...
        >
          ...
          <Box className='flex flex-col gap-1 items-start'>
            ...
            <p className='m-0 font-Montserrat font-bold text-md'>{productName}</p>
          </Box>
          {/* Try On Button */}
          <Box className="p-2 w-[35%] h-4 bg-primary rounded-md flex gap-3 items-center">
            <img className="w-4" src='assets/icons/png/camera.png' alt='camera' />
            <p className='m-0 font-Montserrat font-bold text-sm text-white'>TRY ON</p>
          </Box>
        </Box>
      </Box>
    </Grid>
